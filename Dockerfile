...
ENV MAHOUT_HOME        ${INSTALLATION_PATH}/mahout
ENV MAHOUT_LOCAL       ''
ENV SQOOP_HOME         ${INSTALLATION_PATH}/sqoop
...
ADD conf/apache-mahout-distribution-${MAHOUT_VERSION}.tar.gz /tmp/apache-mahout-distribution-${MAHOUT_VERSION}.tar.gz
ADD conf/sqoop-${SQOOP_VERSION}.tar.gz /tmp/sqoop-${SQOOP_VERSION}.tar.gz
...
RUN cd /tmp && \
    ...
    mv apache-mahout-distribution-${MAHOUT_VERSION}.tar.gz/apache-mahout-distribution-${MAHOUT_VERSION} ${INSTALLATION_PATH} && \
    rm -rf apache-mahout-distribution-${MAHOUT_VERSION}.tar.gz && \
    mv sqoop-${SQOOP_VERSION}.tar.gz/sqoop-${SQOOP_VERSION} ${INSTALLATION_PATH} && \
    rm -rf sqoop-${SQOOP_VERSION}.tar.gz && \
    ...
    ln -s ${INSTALLATION_PATH}/apache-mahout-distribution-${MAHOUT_VERSION}  ${INSTALLATION_PATH}/mahout && \
    ln -s ${INSTALLATION_PATH}/sqoop-${SQOOP_PATH} ${INSTALLATION_PATH}/sqoop

RUN sed -i '/^export JAVA_HOME/ s:.*:export JAVA_HOME=/usr/lib/jvm/jdk\nexport HADOOP_PREFIX=/usr/local/hadoop\nexport HADOOP_HOME=/usr/local/hadoop\n:' $HADOOP_PREFIX/etc/hadoop/hadoop-env.sh
RUN sed -i '/^export HADOOP_CONF_DIR/ s:.*:export HADOOP_CONF_DIR=/usr/local/hadoop/etc/hadoop/:' $HADOOP_PREFIX/etc/hadoop/hadoop-env.sh

RUN echo "export PATH=$PATH:$HADOOP_PREFIX/bin:$HADOOP_PREFIX/sbin:${INSTALLATION_PATH}/bin:$SPARK_HOME/bin:$SCALA_HOME/bin:$HIVE_HOME/bin:$FLUME_HOME/bin:$MAHOUT_HOME/bin:$SQOOP_HOME/bin" >> /root/.bashrc
